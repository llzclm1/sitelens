...
- [x] 将基础分析统一为确定性的规则引擎，报告标记为 `heuristic`，不再调用外部 AI 模型。
- [x] `/pricing` 改为公共 Beta 免费访问说明页；报告不再要求邮箱、账号或付款。
- [x] 更新隐私、条款、README、llms、增长计划和追踪计划，避免旧收费口径继续对外出现。
- [x] 完成类型检查、生产构建、OpenNext 构建、安全审计、供应链审计、依赖审计和 26 页面 Chromium QA。
- [x] 发布 Cloudflare Worker `e86c56b9-c8e0-4125-adf0-7e7d74063a8a`，线上复核免费口径、旧支付接口 404 和分析接口 `mode=heuristic`。
- [ ] 观察 GA4 的分析完成、报告查看、分享和复访数据，不再使用支付漏斗作为当前指标。

## 2026-09-22 30 天真实用户增长启动

- [x] 为分享报告增加品牌 OG 图片和大图社交卡片
- [x] 让桌面端分享复制可直接发送的短句和证据链接
- [x] 生成第三方发布所需的 5 张真实页面截图和证据说明
- [x] 在 GitHub README 展示产品预览并链接 live product
- [x] 录制首页到报告的真实操作演示视频
...
- [x] 报告页增加可复制/原生分享入口和 `share` UTM 归因
- [x] 报告页增加动态社交分享标题/摘要，继续保持报告不被搜索收录
- [x] QA 增加报告分享入口回归检查
- [x] 通过类型检查、Next/OpenNext 构建、源码安全审计、供应链审计和 26 页面 Chromium QA
- [x] 发布 Cloudflare Worker `e12b9f90-5b20-461e-9f07-144fff2318b8`，线上只读核验首页、robots、Sitemap、404 边界和报告分享静态包
- [ ] 按日获取 GA4 用户/来源/事件数据并记录真实增长，不用 Cloudflare 请求替代用户
- [ ] 准备并执行经授权的公开内容、合作分发和目录提交；外部发送状态单独记录
- [x] 准备差异化社区/发布素材包与 UTM 入口；明确 Product Hunt、目录截图/视频和真实评价的待办门槛
- [x] 为公开 Teardown → 分析入口增加按案例区分的 UTM 归因
- [x] 修复 Stripe 独立 Teardown 页遗漏的底部归因入口
...
- [x] 将首次分析收敛为 URL 必填、产品/受众上下文可选，并增加无上下文分析回归
- [x] 线上用 `example.com` 做 URL-only 分析回归：HTTP 201，内部测试不计入真实用户
- [x] 优化 GitHub 公共仓库描述、Homepage 和 topics，增加开发者发现入口
- [x] 启用 GitHub Discussions 并在 README 放置反馈入口
- [x] 在报告页增加反馈社区入口并纳入回归检查
...
- [x] 为没有专属案例入口的意图页补充默认内链：方法边界、公开 Teardown、免费分析；已有相关入口的页面保持原有链接。
...
- [x] 在首页增加 4 个基于用户问题的已有内容入口，不新增重复 SEO 页面
- [x] 为问题入口增加 `ItemList` 结构化数据，明确页面主题与内部链接关系
- [x] 完成类型检查、Next 生产构建、OpenNext 构建和 26 页面 Chromium QA
- [ ] 发布到 Cloudflare 后复核首页结构化数据、问题入口和移动端布局
- [ ] 等待 7–14 天后在 GSC 比较入口页面的展示、查询和 CTR
...
- [x] 为 GA4 事件补充首屏落地页、UTM 白名单、搜索来源和获客渠道上下文
...
Phase 0 可运行原型：验证“免费规则报告 → 分析完成、分享和复访”是否成立。
